...
uint8_t read_record_header(uint8_t record_num, uint32_t *record_id, bool print);
...
uint8_t record_scan_for_new_records(bool forced) {
  ...
  for (i = 0; i < MAX_RECORDS; i++) {
    ...
    pending_status = read_record_header(i, &record_id, forced);
...
uint8_t read_record_header(uint8_t record_num, uint32_t *record_id, bool print) {
  ...
  memset(temp_buffer, 0x00, 36);
  flash_read_buffer(temp_buffer, flash_address, 8);
  buffer = temp_buffer + 4;
  ...
  memset(temp_buffer, 0x00, 36);
  flash_read_buffer(temp_buffer, flash_address, 32);
  buffer = temp_buffer + 4;

  memcpy(record_id, buffer, 4);

  // print header //
  if (print) {
    terminal_buffer_lock();
    sprintf(alert_str, "%d: Record %d: %d-%d-%d %d:%d:%d status: %d\r\n", record_num, *record_id, buffer[7], buffer[8], buffer[9], buffer[4], buffer[5], buffer[6], pending_status);
    DisplayMessage(alert_str, strlen(alert_str), false);
    terminal_buffer_release();
  }
...
void record_print(uint8_t record_num) {
  ...
  read_record_header(record_num, &value, true);
