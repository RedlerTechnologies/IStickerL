...
void FileTransferFailed(bool abort) {
  ...
  if (ble_services_is_connected() & !abort) {
    if (ble_reading_file_state.record_num >= 0) {
      ...
      CreateDebugEvent(EVENT_DEBUG_RECORD_TRANSMIT_FAILED, ble_reading_file_state.record_id, false);
    }
  }
...
static void FileTransferSucceed(void) {
  if (ble_reading_file_state.record_num >= 0) {
    ...
    CreateDebugEvent(EVENT_DEBUG_RECORD_TRANSMIT_SUCCEED, ble_reading_file_state.record_id, false);
  }
...
void BFT_start(unsigned short record_num, unsigned char is_record) {
  ...
  if (is_record) {
    ...
    CreateDebugEvent(EVENT_DEBUG_RECORD_TRANSMIT_STARTED, ble_reading_file_state.record_id, false);
  } else {
